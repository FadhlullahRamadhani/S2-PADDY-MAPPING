...
#
library(readxl)
area_STATS <- read_excel("C:/MasseyOffice1/Research/R-Script2019/area_kab.xlsx")
  ...
  for (j in 1:length(idkab)){
    ...
    for (time in time_list) {
      ...
      for (period in period_list) {
        ...
        if (region=="3212") {
          datestart <- as.Date("2018-06-01")
          period_date <- as.Date(period_first)
          diff <- as.numeric(period_date - datestart)
          if (diff<0) {
            next
          }
          if (diff %% 5 !=0) {
        ...
        if (region %in% c("3213","3215")) {
          datestart <- as.Date("2018-06-04")
          period_date <- as.Date(period_first)
          diff <- as.numeric(period_date - datestart)
          if (diff<0) {
            next
        ...
        if (region %in% c("3517","3518","3522","3524")) {
          datestart <- as.Date("2018-06-03")
          period_date <- as.Date(period_first)
          diff <- as.numeric(period_date - datestart)
          if (diff<0) {
            next
        ...
        mainDir_results_S2_csv <- paste("F:/ML-S2_S1v1/CLASSIFY-MASK-PADDY-CLEAR-CD/CLASSIFY-MASK-PADDY-CLEAR-CD-",time,"/",period,"/",model_ML,"/",collapse = "",sep = "")
...
filename_csv_paddy_subdistrict <- paste(mainDir_results,"Recap-Change_V4_result_kab.csv", sep="")
...
filename_csv_paddy_subdistrict <- paste(mainDir_results,"Recap-Change_V4_result_prov.csv", sep="")
...
filename_csv_paddy_subdistrict <- paste(mainDir_results,"Recap-Change_V4_result_root.csv", sep="")
