...
library(readxl)
S1_MOD13Q1_ML_v6 <- read_excel("C:/MasseyOffice1/Research/R-Script2020/S1_MOD13Q1_ML_v6.xlsx")

S1_MOD13Q1_ML_v6$Phase <- as.factor(S1_MOD13Q1_ML_v6$Phase)
set.seed(7)
# 
# NR <- filter(S1_MOD13Q1_ML_v6, Phase == "1")
# VEG <- filter(S1_MOD13Q1_ML_v6, Phase == "2")
# REPR <- filter(S1_MOD13Q1_ML_v6, Phase == "3")
# RIPE <- filter(S1_MOD13Q1_ML_v6, Phase == "4")
# 
...
# 
# S1_MOD13Q1_ML_v6_sampling_down <- rbind(NR_sample, VEG_sample,REPR_sample,RIPE_sample)
# S1_MOD13Q1_ML_v6_sampling_down <-  S1_MOD13Q1_ML_v6_sampling_down[sample(1:nrow(S1_MOD13Q1_ML_v6_sampling_down)), ]
S1_MOD13Q1_ML_v6_sampling_down <- S1_MOD13Q1_ML_v6
rice_fmask <- S1_MOD13Q1_ML_v6_sampling_down
trainIndex <- createDataPartition(rice_fmask$Phase, p=0.9, list=FALSE)
train.data <- rice_fmask[ trainIndex,]
test.data <- rice_fmask[-trainIndex,]
# write.csv(train.data,file="S1_MOD13Q1_ML_v6 train data ML.csv")
# write.csv(test.data,file="S1_MOD13Q1_ML_v6 test data ML.csv")

rice <-S1_MOD13Q1_ML_v6_sampling_down
rice$PHASE <- factor(rice$Phase)
print(summary(rice))



allModelsList <- c( "Phase ~ NDVI + EVI",
                    "Phase ~ TIME2 + TIME1 + TIME0 + NDVI",
                   "Phase ~ TIME1 + TIME0 + NDVI",
                   "Phase ~ TIME0 + NDVI",
                   "Phase ~ TIME2 + TIME1 + TIME0 + EVI",
                   "Phase ~ TIME1 + TIME0 + EVI",
                   "Phase ~ TIME0 + EVI",
                   "Phase ~ TIME2 + TIME1 + TIME0",
                   "Phase ~ TIME1 + TIME0",
                   "Phase ~ TIME0",
                   "Phase ~ NDVI",
                   "Phase ~ EVI",
                   "Phase ~ TIME2 + TIME1 + TIME0 + NDVI  + EVI",
                   "Phase ~ TIME1 + TIME0 + NDVI + EVI",
                   "Phase ~ TIME0 + NDVI + EVI",
                   "Phase ~ TIME2 + TIME1 + TIME0 + NDVI + NDVI_1",
                   "Phase ~ TIME1 + TIME0 + NDVI + NDVI_1",
                   "Phase ~ TIME0 + NDVI + NDVI_1",
                   "Phase ~ TIME2 + TIME1 + TIME0 + EVI + EVI_1",
                   "Phase ~ TIME1 + TIME0 + EVI + EVI_1",
                   "Phase ~ TIME0 + EVI + EVI_1",
                   "Phase ~ TIME2 + TIME1 + TIME0",
                   "Phase ~ TIME1 + TIME0",
                   "Phase ~ TIME0",
                   "Phase ~ NDVI + NDVI_1",
                   "Phase ~ EVI + EVI_1",
                   "Phase ~ TIME2 + TIME1 + TIME0 + NDVI + EVI + NDVI_1 + EVI_1",
                   "Phase ~ TIME1 + TIME0 + NDVI + EVI + NDVI_1 + EVI_1",
                   "Phase ~ TIME0 + NDVI + EVI + NDVI_1 + EVI_1",
                   "Phase ~ NDVI + EVI + NDVI_1 + EVI_1",
                   ...
                   "Phase ~ TIME0 + DTNDVI",
                   "Phase ~ TIME0 + DTEVI",
                   "Phase ~ TIME0 + DTNDVI + DTEVI",
                   "Phase ~ TIME0 + NDVI + DTNDVI",
                   "Phase ~ TIME0 + EVI + DTEVI",
                   
                   "Phase ~ TIME1 + TIME0 + DTNDVI",
                   "Phase ~ TIME1 + TIME0 + DTEVI",
                   "Phase ~ TIME1 + TIME0 + DTNDVI + DTEVI",
                   "Phase ~ TIME1 + TIME0 + NDVI + DTNDVI",
                   "Phase ~ TIME1 + TIME0 + EVI + DTEVI"
...
mainDir <- paste("C:/MasseyOffice1/Research/R-Script2020/S1_MOD13Q1_ML_v6/",collapse = "",sep = "")
...
for (model_ML in model_filter) {
  ...
  for(i_formula in 1:length(allModelsList)) {
    ...
    for(i_preName in 1:1) {
      ...
      filename <- paste(name_fmask,"_",name_data,"_", "S1_MOD13Q1_ML_v6_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      ...
      filename_save <- paste(name_fmask,"_",name_data,"_", "S1_MOD13Q1_ML_v6_",model_ML,"_",preName_current_string,"_",numbering,".rds", sep="")
      ...
      if (any(model_varimp == model_ML) ) {
        ...
        filename_varImp <- paste("varImp_",name_fmask,"_",name_data,"_", "S1_MOD13Q1_ML_v6_",model_ML,"_",preName_current_string,"_",numbering,"_all.csv", sep="")
      ...
      filename_rfc_overall <- paste("ConMat_overall_",name_fmask,"_",name_data,"_", "S1_MOD13Q1_ML_v6_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      filename_rfc_table <- paste("ConMat_table_",name_fmask,"_",name_data,"_", "S1_MOD13Q1_ML_v6_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      filename_rfc_class <- paste("ConMat_class_",name_fmask,"_",name_data,"_", "S1_MOD13Q1_ML_v6_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      ...
      if (any(model_varimp == model_ML) ) {
        allModelsResults_varImp <- varImp(allModelsResults)
        allModelsResults_varImp_1 <- allModelsResults_varImp$importance
        filename_varImp <- paste("varImp_",name_fmask,"_",name_data,"_", "S1_MOD13Q1_ML_v6_",model_ML,"_",preName_current_string,".csv", sep="")
        write.csv(allModelsResults_varImp_1, file = filename_varImp)
      }
