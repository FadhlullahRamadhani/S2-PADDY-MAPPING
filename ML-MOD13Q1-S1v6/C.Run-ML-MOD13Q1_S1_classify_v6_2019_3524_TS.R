    ...
    if (file.exists(filename_tif)==FALSE) {
      ...
      for (intVh in 1:length(files_VH)) {
        ...
        if ((date_VH>=period-20) & (as.Date(date_VH) <= as.Date(period+10))) {
          ...
          if (minValue(check_raster)<1.797693e+308) {
      ...
      raster_TIME0 <-  merge(VHrasterlist[[3]],VHrasterlist[[4]],tolerance=10)
      raster_TIME1 <- merge(VHrasterlist[[1]],VHrasterlist[[2]],tolerance=10)
